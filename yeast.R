# Generate all the required data for the yeast expression processing
...
cosPairwiseFilename = "yeast_results/cos.csv"
micePairwiseFilename = "yeast_results/mice.csv"
dcorPairwiseFilename = "yeast_results/dcor.csv"
rdcPairwiseFilename = "yeast_results/rdc.csv"
ccorPairwiseFilename = "yeast_results/ccor.csv"

# read in the data
yeastData = read_csv("yeast_results/data.csv")
numFeatures = ncol(yeastData)
...
if(!file.exists(cosPairwiseFilename)) {
  printf("Computing CoS Pairwise Dependencies for Yeast Data")
  pairwiseMat = matrix(0, nrow=numFeatures, ncol=numFeatures)
  for(ii in c(1:numFeatures)) {
    for(jj in c(ii:numFeatures)) {
      if(ii!=jj) {
        # a symetric matrix ...
        pairwiseMat[ii,jj] = cosdv(yeastData[[ii]], yeastData[[jj]])
        pairwiseMat[jj,ii] = cosdv(yeastData[[ii]], yeastData[[jj]])
      }
    }
  }
  # write to file
...
if(!file.exists(micePairwiseFilename)) {
  printf("Computing MICe Pairwise Dependencies for Yeast Data")
  pairwiseMat = matrix(0, nrow=numFeatures, ncol=numFeatures)
  for(ii in c(1:numFeatures)) {
    for(jj in c(ii:numFeatures)) {
      if(ii!=jj) {
        # a symetric matrix ...
        pairwiseMat[ii,jj] = mine(yeastData[[ii]], yeastData[[jj]])$MICe
        pairwiseMat[jj,ii] = mine(yeastData[[ii]], yeastData[[jj]])$MICe
      }
    }
  }
  # write to file
...
if(!file.exists(dcorPairwiseFilename)) {
  printf("Computing dCor Pairwise Dependencies for Yeast Data")
  pairwiseMat = matrix(0, nrow=numFeatures, ncol=numFeatures)
  for(ii in c(1:numFeatures)) {
    for(jj in c(ii:numFeatures)) {
      if(ii!=jj) {
        # a symetric matrix ...
        pairwiseMat[ii,jj] = dcor(yeastData[[ii]], yeastData[[jj]])
        pairwiseMat[jj,ii] = dcor(yeastData[[ii]], yeastData[[jj]])
      }
    }
  }
  # write to file
...
if(!file.exists(rdcPairwiseFilename)) {
  printf("Computing RDC Pairwise Dependencies for Yeast Data")
  pairwiseMat = matrix(0, nrow=numFeatures, ncol=numFeatures)
  for(ii in c(1:numFeatures)) {
    for(jj in c(ii:numFeatures)) {
      if(ii!=jj) {
        # a symetric matrix ...
        pairwiseMat[ii,jj] = rdc(yeastData[[ii]], yeastData[[jj]])
        pairwiseMat[jj,ii] = rdc(yeastData[[ii]], yeastData[[jj]])
      }
    }
  }
  # write to file
...
if(!file.exists(ccorPairwiseFilename)) {
  printf("Computing cCor Pairwise Dependencies for Yeast Data")
  pairwiseMat = matrix(0, nrow=numFeatures, ncol=numFeatures)
  for(ii in c(1:numFeatures)) {
    for(jj in c(ii:numFeatures)) {
      if(ii!=jj) {
        # a symetric matrix ...
        pairwiseMat[ii,jj] = rcd(yeastData[[ii]], yeastData[[jj]])
        pairwiseMat[jj,ii] = rcd(yeastData[[ii]], yeastData[[jj]])
      }
    }
  }
  # write to file
...
trueNetwork = read.table("yeast_results/true.txt",header=F)
